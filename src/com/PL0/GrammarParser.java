...
public class GrammarParser {
    ...
    public GrammarParser(String filePath) {
        ...
        S = new int[stackSize];
        //变量初始化
        lexicalAnalyzer = new LexicalAnalyzer(FileUtil.readFileContent(filePath));
    ...
    public void pl0() {
        ...
        if (!SYM.equals("period")) errors.add(PL0Error.log(9)); //主程序分析结束，应遇到表明程序结束的句号
        ...
        if (errors.size() == 0) interpret();
        else {
            System.out.println("PL/0程序中的错误：");
            for (String error : errors) {
                System.out.println(error);
            }
        }
        int k = 0;
        System.out.println("PL/0程序中的单词表：");
        for (Token token : tokens) {
            System.out.println((k++) + " " + token.toString());
        }
        k = 0;
        System.out.println("PL/0程序中的声明表：");
        for (Declaration declaration : declarations) {
            System.out.println((k++) + " " + declaration.toString());
        }
        k = 0;
        System.out.println("PL/0程序中的代码表：");
        for (Code code : codes) {
            System.out.println((k++) + " " + code.toString());
        }
    }
    ...
    private void block(Integer level, String[] FSys) {
        ...
        do {
            if (SYM.equals("constSym")) { //token是const保留字，开始进行常量声明
                ...
                if (SYM.equals("identifier")) {
                    ...
                    if (SYM.equals("semicolon")) { //如果常量声明结束，应遇到分号
                        ...
                        errors.add(PL0Error.log(5)); //漏掉了.或;
                    }
                }
            }
            ...
            while (SYM.equals("procedureSym")) {
                ...
                if (SYM.equals("identifier")) {
                    ...
                    errors.add(PL0Error.log(4)); //应是标识符
                }
                if (SYM.equals("semicolon")) { //如果常量声明结束，应遇到分号
                    getToken();
                } else {
        ...
        test(ArrayUtils.addAll(statBegSys, "identifier".split("@")), declBegSys, 7);
        ...
        switch (SYM) {
            ...
            case "readSym" -> { //read语句
                ...
                if (!SYM.equals("lparen")) {
                    errors.add(PL0Error.log(34)); //read保留字后应接(
                } else {
                    do {
                        ...
                        if (i == -1) errors.add(PL0Error.log(35)); //read语句中标识符应为变量
                        else if (i == 0) errors.add(PL0Error.log(36)); //read语句括号中应接变量标识符
                        else {
                    ...
                    if (!SYM.equals("rparen")) {
                        errors.add(PL0Error.log(33)); //read语句的后跟符应为)
            ...
            case "writeSym" -> { //write语句
                getToken(); //获得下一token，正常情况下应为左括号
                if (!SYM.equals("lparen")) {
                    errors.add(PL0Error.log(34)); //write保留字后应接(
                } else {
                    do {
            ...
            case "callSym" -> { //call语句
                ...
                if (SYM.equals("identifier")) {
                    ...
                    if (i == 0) errors.add(PL0Error.log(11)); //标识符未说明
                        ...
                        else errors.add(PL0Error.log(15)); //call后标识符属性应为过程
                    }
                    getToken();
                } else errors.add(PL0Error.log(14)); //call后应为标识符
            }
            case "ifSym" -> { //if语句
                ...
                } else errors.add(PL0Error.log(16)); //条件语句中丢了then
            ...
            case "beginSym" -> { //begin语句
                ...
                while (Arrays.asList(ArrayUtils.addAll(statBegSys, "semicolon".split("@"))).contains(SYM)) {
                    ...
                    else errors.add(PL0Error.log(10)); //语句之间漏了;
                    statement(level, FSys);
                }
                if (SYM.equals("endSym")) { //如果语句全分析完了，应该遇到end
                    getToken();
                } else {
                    errors.add(PL0Error.log(17)); //丢了end或;
                }
            }
            case "whileSym" -> { //while语句
                ...
                if (SYM.equals("doSym")) { //表达式后应为do语句
                    getToken(); ////获得下一token，正常情况下应为语句
                } else errors.add(PL0Error.log(18)); //while型循环语句中丢了do
    ...
    private void condition(Integer level, String[] FSys) {
        ...
        if (SYM.equals("oddSYM")) { //odd运算符(一元)
            ...
            if (!Arrays.asList(condBegSys).contains(SYM)) { //token不是关系运算符
                errors.add(PL0Error.log(20)); //应为关系运算符
            } else {
            ...
            switch (SYM) {
                case "identifier" -> { //遇到的是标识符
                    ...
                    if (i == 0) { //如果查声明表返回为0，表示没有找到标识符
                        errors.add(PL0Error.log(11)); //标识符未说明
                    } else {
                        ...
                        switch (declaration.getKind()) {
                            ...
                            case "procedureSym" -> errors.add(PL0Error.log(21)); //表达式内标识符属性不能是过程
                        }
                    }
                    getToken();
                }
                case "number" -> { //遇到的是数字
                    if (NUM > NumMAx) {
                        errors.add(PL0Error.log(31)); //数越界
                        NUM = 0; //将数字按0值处理
                    }
                ...
                case "lparen" -> { //遇到的是左括号
                    ...
                    if (SYM.equals("rparen")) { //子表达式分析完后，应遇到右括号
                        getToken();
                    } else errors.add(PL0Error.log(22)); //表达式中漏掉右括号)
                }
            }
        }
        ...
        if (SYM.equals("identifier")) { //常量声明过程开始遇到的第一个符号必然应为标识符
            ...
            if (SYM.equals("eql") || SYM.equals("becomes")) {
                if (SYM.equals("becomes")) { //如果是赋值号(常量生明中应该是等号
                    errors.add(PL0Error.log(1)); //常数说明中的=写成:=
                }
                getToken();
                if (SYM.equals("number")) {
                    ...
                    errors.add(PL0Error.log(2)); //常数说明中的=后应是数字
                }
            } else {
                errors.add(PL0Error.log(3)); //常数说明中的标识符后应是=
            }
        } else {
            errors.add(PL0Error.log(4)); //应是标识符
        }
